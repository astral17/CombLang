...
/** Returns the lowercase SHA-256 digest of a UTF-8 string synchronously. */
export function sha256Utf8Hex(value: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      schedule[index] =
        (schedule[index - 16]! + lowerSigma + schedule[index - 7]! + upperSigma) >>> 0;
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      const bigSigmaUpper = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25);
      const choose = (e & f) ^ (~e & g);
      const first = (h + bigSigmaUpper + choose + ROUND_CONSTANTS[index]! + schedule[index]!) >>> 0;
      const bigSigmaLower = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22);
      const majority = (a & b) ^ (a & c) ^ (b & c);
      const second = (bigSigmaLower + majority) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + first) >>> 0;
