...
- [ ] Phase 6 — universal persistent Factorio Entity handles with a generic modded fallback and typed facades.
  ...
  - [x] Expose the host-embedding source subset `Entity(prototype)`, `entity.port(connector, lane)`, and `entity.bind(connector, lane, network, direction)` through v3 replay, blueprint, debug, simulation, and generic test adapters; the default website and ordinary CLI profile selection do not install reviewed Entity profiles, so no out-of-the-box construction, configuration, placement, or native-verified claim is made.
