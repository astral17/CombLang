...
- [ ] Phase 5.5 — external prototype environment foundation
  ...
  - [x] Keep color consistency checks inside executed elaboration operations, preserving first-conflict spans, separating logical Network union from equal-color parity, and preventing later DSL execution after a contradiction.
- [ ] Phase 6 — universal persistent Factorio Entity handles with a generic modded fallback and typed facades: shared circuit inputs, native single-comparison `enable`, Roboport, Lamp, Constant, logistics entities, belts, displays, and train stops.
  ...
  - [x] Allow transparent `Network` references, untyped/`any` exact-value parameters, and recursive `Network` unions with runtime branch selection across declarations, arrows, and function expressions. Bare references do not borrow or move; explicit `Readonly`, `Ref`, and `Move` retain their contracts, while `CL2002` warns once per executed unrestricted Network parameter declaration.
