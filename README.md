...
- [ ] Phase 6 — universal persistent Factorio Entity handles with a generic modded fallback and typed facades.
  ...
  - [x] Expose the host-embedding source subset `Entity(prototype)`, `entity.port(connector, lane)`, and `entity.bind(connector, lane, network, direction)` through v3 replay, blueprint, debug, simulation, and generic test adapters; selected built-in and imported providers now provision conservative zero-port fallback profiles only for records whose normalized `blueprintEligible` fact is explicitly true, while typed configuration and native-verified claims remain gated by reviewed profiles.
  - [x] Add the bounded, profile-free `Entity(prototype, { raw })` BlueprintEntity escape hatch with compiler-owned identity/topology fields kept under compiler control; native validity and simulation remain outside this untyped path.
  - [x] Add schema-checked partial `BlueprintEntity` fragments from provider prototype `type`, including common-only types and detached session `Signal(...)` values; lower successful checks to the existing raw physical payload without native claims.
  - [x] Transport successful host-authorized v3 lowering as a profile-free, cloneable `ResolvedSourceCircuit`; main-thread blueprint/simulation hydration never replays the Entity plan or grants new construction authority.
  - [x] Extend that host-embedding subset with public `{ rule, lanes, condition: NativeCondition(...) }` configuration and `.at(x, y, direction?)`; the configuration lowers to the existing typed native single-condition field without a hidden Decider, tick, Producer, or Network.
  - [x] Add the structural `Lamp(prototype, configuration?)` family facade over generic Entity construction, preserving one nominal/physical Entity and requiring provider type `lamp` without inferred connectors, callable behavior, or native claims.
  - [x] Add the structural `Roboport(prototype, configuration?)` family facade over generic Entity construction, preserving one nominal/physical Entity and requiring provider type `roboport` without inferred readback, connectors, callable behavior, or native claims.
  - [x] Add the structural `Constant(prototype, configuration?)` family facade over generic Entity construction, preserving one nominal/physical Entity and keeping `CC(...)` as the separate executable ConstantCombinator path without unification claims.
