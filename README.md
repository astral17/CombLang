...
- [ ] Phase 6 — universal persistent Factorio Entity handles with a generic modded fallback and typed facades.
  ...
  - [ ] Add the remaining fixture-backed typed facades in order: Roboport, then `Constant(...)` as an Entity while `CC(...)` remains a Combinator and both share one physical implementation record.
