...
const lamp = Lamp('fixture-lamp');`);
  ...
  test('accepts bounded raw Entity configuration through the selected CLI provider', async () => {
    const source = await sourceFile(`const machine = Entity('assembling-machine-3', {
  raw: {
...
const machine = Entity('assembling-machine-3', {
  ...
  test('constructs a Lamp facade through the selected CLI provider', async () => {
    ...
    const source = await sourceFile(
      `const lamp = Lamp('fixture-lamp', { always_on: false }).at(2, 3, 8);`,
    );
    const profile = await profileFile(JSON.stringify(database));
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
  ...
  test('checks exact Arithmetic through the selected CLI provider as cumulative v5', async () => {
    ...
    database.entities.push({
      key: 'entity:arithmetic-combinator',
    ...
    const source = await sourceFile(
      `const A = Signal('virtual', 'signal-A');
const input = new Network();
const exact: ArithmeticCombinator = Arithmetic({ left: input[A], operation: 'multiply', right: 2, output: A }).at(2, 3, 8);
const output = new Network();
output += exact;`,
    );
    const profile = await profileFile(JSON.stringify(database));
