...
The runtime value shapes and their nominal `RuntimeValueRegistry` now live in a dedicated internal module rather than in the execution coordinator. This is the first extraction from that coordinator; ownership transitions and operator dispatch still remain there until their interfaces can be separated without duplicating policy.

Every Network input reference crossing the direct-plan, EG, or NCIR boundary is explicitly discriminated as either `{ refKind: "single", network }` or `{ refKind: "pair", networks: [first, second] }`. A pair has no synthetic primary Network: all consumers must handle both colors intentionally. This incompatible schema cleanup advances `comblang-direct-plan`, `comblang-eg`, and `comblang-ncir` to version 2; the elaboration-JavaScript envelope remains version 1 because its executable-module contract did not change.
...
Phase 3's executed compiler path is complete. Phase 4 is underway: zero-tick consuming transfer, function-scoped `Readonly`/`Ref` borrow views, explicit `Move<Network>` call/return ownership, shared-generation aliases with ordinary container-slot replacement, and immutable `pair(a, b)` both-colors input views are executable. These features extend the ordinary-JavaScript execution model rather than replacing it. Successful capability uses now survive as audit descriptors beside `networkTransfers` and `networkPairs` through the serialized direct plan, validated EG/NCIR execution result, CLI JSON, and browser plan. They do not create hardware; their physical consequences are already reflected in color and topology constraints. The remaining work is the final acceptance-coverage audit; the design and criteria live in [Phase 4: ownership and multi-network design](ownership-and-multi-network.md).
