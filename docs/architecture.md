...
Public DSL annotations are parsed once in the language package. Semantic preflight and executable transform consume the same small `DslTypeSyntax` model for owned/borrowed/moved Networks, color requirements, concrete Combinator handles, and array containers. This parser intentionally recognizes only the stable DSL surface; it does not attempt TypeScript assignability or general symbol resolution. `Producer` remains a deprecated source alias for `Combinator` during migration.
...
`combinator-output-policy` owns Signal rebinding independently from connection syntax. It mutates the central physical descriptor, retains the original Each input selection, and emits `RT2023` with creation provenance for constant, wildcard, multi-output, else-branch, or conflicting Signal bindings. Every alias observes the same configuration.
...
`CombinatorRegistry` replaces `ProducerLifecycle`. It follows one physical identity across aliases and typed wrappers, owns capture IDs, shared mutable configuration, output-lane state, placement, and usage tracking, and serializes each device exactly once only after synchronous execution completes.
