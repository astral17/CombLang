...
Normalized producer input traversal is centralized in `producer-network-references`. Its occurrence view retains descriptor order and repeated normal/else Decider output rows; its topology view expands pair references and returns frozen distinct Network IDs in first-use order. Color constraints, debug graph analysis, and native blueprint wiring consume that topology view instead of maintaining separate condition/output walkers. The temporary browser depth heuristic still traverses the pre-NCIR Direct Plan and is replaced by resolved graph/SCC analysis in F08.

`CircuitColorConstraints` owns the compiler's stateful parity DSU independently of source execution and presentation code. Networks receive stable registration ordinals; `same`, `different`, and fixed-color constraints are checked as they are added, retain their reason/provenance on conflicts, and do not implicitly register unknown IDs. `resolve` is observational: anchored components follow their fixed color, while the earliest registered member of every free component is deterministically red regardless of union order. The batch `solveCircuitColors` API is a compatibility wrapper over this engine.

The executed source recorder owns one such engine for its complete lifetime. Its opaque physical Network identity is the ownership-state object rather than the mutable display name, so aliases, function-return rebinding, moves, and zero-tick `take` do not lose color history. Network creation/fixed requirements, pair creation, producer input connectors, output attachment, and `take` register constraints immediately. The first contradiction is therefore attributed to the source operation that closes the inconsistent prefix instead of being deferred to Direct Plan replay.
...
`producer-output-policy` owns this rebinding independently from attachment syntax. It preserves the Producer identity, rewrites only the native output descriptor, retains the original Each input selection, and emits `RT2023` with creation provenance for constant, wildcard, multi-output, else-branch, or conflicting Signal bindings. The recorder supplies nominal branding and concrete destination provenance but does not duplicate the compatibility matrix.
...
Producer identity bookkeeping is isolated in `ProducerLifecycle`. It follows one physical identity across fluent and typed wrappers, owns capture IDs and the first attachment source, remembers the emitted plan index, and finalizes unused identities exactly once. A debug capture performed after attachment updates that existing descriptor instead of losing the Producer-to-runtime mapping; it never emits a second physical combinator.

`producer-handle-policy` is the category gate for explicit `Producer`, `ArithmeticCombinator`, `DeciderCombinator`, and `ConstantCombinator` annotations. A mismatch is `RT2022`; a valid named binding receives a nominally branded wrapper with the same opaque identity and an updated debug `bindingName`. Unknown annotation metadata and non-string binding metadata are transform/runtime contract failures rather than user category mismatches.

`network-argument-policy` owns the executed call-argument category boundary. It either accepts an opaque Network handle or contextually materializes a Producer under the stable `$argument:<function>:<parameter>` name, rejects every other executed value with source-linked `RT2015`, validates readability before creating a callee view, and carries the direct argument expression span into the later borrow/move check. Borrow lifetime, ownership mutation, color requirements, and plan capability-use recording remain separate recorder/ownership operations.
...
`network-return-policy` owns the typed `Network`/`Readonly<Network>` return category. It accepts an existing Network or contextually materializes a Producer into the reserved `$return` Network, applies any declared color requirement with transfer/read-only capability, delegates the actual owner transfer to `elaboration-ownership`, and only then brands an optional read-only caller view. This single-value typed path is distinct from `return-owned-value-policy`, which validates and atomically replaces Networks discovered inside an unannotated array/plain-object return graph.

`producer-attachment-policy` validates the one physical output connector before any plan descriptor is emitted: at least one and at most two distinct destination Networks, one attachment per Producer identity, then writable capability for every destination. These failures remain `RT2003` through `RT2006` with declaration, creation, and prior-attachment spans. The recorder only supplies lifecycle state and the capability operation before binding the output Signal and serializing destinations.
