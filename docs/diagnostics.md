...
`instancePath`, and grouped diagnostics carry `occurrences` plus bounded
`instancePaths`. Runtime color conflicts may also include related Network
declarations.
...
The transport keeps display codes stable and uses four semantic severities:
`error`, `warning`, `note`, and `hint`. Default visibility is enabled for
`error`, `warning`, and `note`, and disabled for `hint`. Editors with only three
visual levels render `note` and `hint` as informational decorations while
retaining their semantic level in the diagnostic data and text.

The current advisory registry is:

| Rule ID                                   | Code     | Category    | Default | Grouping support |
| ----------------------------------------- | -------- | ----------- | ------- | ---------------- |
| `producer.unused-output`                  | `CL2001` | correctness | warning | source site      |
| `function.unrestricted-network-parameter` | `CL2002` | ownership   | warning | none             |

Project JSON may contain an optional, exact `diagnostics` object. It is
cloneable data; accessors, symbols, arrays, unknown fields, invalid rule IDs,
and unreasonable provenance bounds are rejected:
...
`levels` controls visibility for every diagnostic at that level. A rule
override may set `enabled`, promote or demote an advisory with `severity`, and
opt into `group`ing. Errors are immutable policy boundaries: they remain
visible and cannot be downgraded or disabled. Promoting an advisory to
`error` makes the CLI and test command fail with a non-zero status.

Grouping is deterministic and keyed by rule ID plus source site. The first
message/span is retained, `occurrences` records the total, and at most
`maxInstanceDetails` distinct generated `instancePaths` are returned. Different
source sites remain separate; diagnostics without an opt-in grouping rule are
never deduplicated.

Policy is applied once at the final source-compilation artifact boundary. Raw
parser, semantic, execution, and lowering decisions still control whether
source executes, so hiding an error cannot make invalid source run. The CLI and
browser receive the same resolved `pipelineDiagnostics` and
`compilerDiagnostics`; parser diagnostics remain in the pipeline view and are
not filtered by advisory policy.

The planned `decider.each-concrete-copy` advisory is intentionally not emitted
yet. Its legal native behavior and explicit-intent spelling remain gated by
exported Factorio fixtures; this configurable foundation does not claim that
native semantics.
